...
export function SportsList(props){
    ...
    let sportArray = props.sportArray
                ...
                <div>
                    <div className='primary__menu__component'>
                        <div className={!props.LiveArrowStatusResult ? 'menu__background__on info' : 'menu__background__off info'}>
                            ...
                            <i 
                                onClick={props.LiveArrowStatusToState} 
                                id={live.key} 
                                className={!props.LiveArrowStatusResult ? "arrow fa fa-angle-down" : " minus fa fa-minus"}>
                            </i>
                        </div>
                ...
                <React.Fragment key={e.key}>
                    <div className='primary__menu__component'>
                        <div className={props.ArrowResult.indexOf(e.key) !== -1 ? 'menu__border__on info' : 'menu__border__off info'}>
                            ...
                            <p>{e.label}</p>
                            <i 
                                onClick={props.setArrowStatusToState} 
                                id={e.key} 
                                className={props.ArrowResult.indexOf(e.key) === -1 ? "arrow fa fa-angle-down" : " minus fa fa-minus"}>
                            </i>
                        </div>
                        <div className={props.ArrowResult.indexOf(e.key) !== -1 ? 'menu__border__on views' : 'menu__border__off views'}>
                            ...
                            <span>
                                <i 
                                    onClick={props.SetSelectedSportKeyToState} 
                                    id={e.key} 
                                    className={elementKeyFromState.indexOf(e.key) !== -1 ? "star checked fa fa-star" : "star fa fa-star"}>
                                </i>
                            </span>
                        </div>
                    </div>
                    {
                        props.ArrowResult.indexOf(e.key) !== -1 
